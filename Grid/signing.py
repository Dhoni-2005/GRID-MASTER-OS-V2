...
_HEARTBEAT_FIELDS: tuple[str, ...] = (
    "node_id", "timestamp_utc", "active_task_ids"
)


def sign_heartbeat(payload: dict, secret: str) -> str:
    """
    Compute HMAC-SHA256 signature for a HeartbeatPayload dict.

    Parameters
    ----------
    payload : HeartbeatPayload.to_dict() (signature field excluded from input)
    secret  : NODE_SECRET shared between master and worker
    ...
    """
    if not secret:
        raise ValueError("Cannot sign heartbeat: NODE_SECRET is empty")
    # active_task_ids is a list — normalise to sorted comma-joined string
    normalised = dict(payload)
    ids = normalised.get("active_task_ids", [])
    normalised["active_task_ids"] = ",".join(str(i) for i in sorted(ids))
    canonical = _canonical_string(normalised, _HEARTBEAT_FIELDS)
    return _compute_hmac(canonical, secret)


def verify_heartbeat(payload: dict, signature: str, secret: str) -> bool:
    """
    Verify an HMAC-SHA256 signature for a HeartbeatPayload dict.
    Uses hmac.compare_digest() for timing-safe comparison.

    Returns True if valid, False otherwise. Never raises.
    """
    if not secret or not signature:
        log.warning("verify_heartbeat: empty secret or signature")
        return False
    try:
        expected = sign_heartbeat(payload, secret)
        return hmac.compare_digest(expected, signature)
    except Exception as exc:
        log.warning("verify_heartbeat: unexpected error: %s", exc)
        return False
